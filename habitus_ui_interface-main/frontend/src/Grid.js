...
function GridCell({id, data, rowName, colName, onChange, onDrop, activateCell, isActive, apiUrl}){
    ...
    const gradientArray = ['#f0f7fd','#cce6fe','#a9d3ff','#87c1ff','#65adff','#4099ff','#0084ff','#0084ff','#117bf3','#1972e6','#1e69da','#2160ce','#2258c2','#234fb6','#2247aa','#213f9f','#203793','#1e2f88','#1b277d','#182071','#151867','#11115c', '#0f1159','#0c1057','#0a0f54','#080f51','#060e4e','#040e4c','#030d49','#020c46','#010b43','#010941','#01083e','#01063b','#020439','#020236','#020236','#020236','#020236','#020236','#020236','#020236','#020236','#020236']
...
function GridRow({rowName, data, onChange, onDrop, activateCell, activeCell, apiUrl}){

    let cells = Object.entries(data).map(([colName, v], ix) => <GridCell key={ix} id={rowName+colName} data={v} rowName={rowName} colName={colName} onChange={onChange} onDrop={onDrop} activateCell={activateCell} isActive={activeCell === rowName+colName} apiUrl={apiUrl} />)
...
function Footer({id, colName, col_num_to_name, onFooter, apiUrl}){
    // console.log(id, ": ", colName);
    return (
    <td key={id}><div contenteditable="true" onInput={
            (evt) => {
...
export default function Grid({data, col_num_to_name, onChange, onDrop, onFooter, apiUrl}){
    const [activeCell, setActiveCell] = useState();
    const activateCell = (item) => activeCell === item ? setActiveCell() : setActiveCell(item);

    let gridRows = Object.entries(data).map(([name, cells], ix) => <GridRow key={ix} rowName={name} data={cells} onChange={onChange} onDrop={onDrop} activateCell={activateCell} activeCell={activeCell} apiUrl={apiUrl} />)
    ...
    if(rows.length > 0){
        ...
        footer = colNames.map((name, ix) => <Footer key = {ix} id={ix} colName={name} onFooter={onFooter} apiUrl={apiUrl} />)
    }
