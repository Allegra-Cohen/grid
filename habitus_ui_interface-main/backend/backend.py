...
class Backend():
	...
	def get_grid(self, flag: str, k: int, anchor: str, filename: str) -> Grid:
		print("New grid -- processing documents ... ")
		self.set_up_corpus(filename, anchor)
		grid = Grid.generate(flag, self.path, filename, self.corpus, k, self.synonym_book)
		return grid


	def load_grid(self, flag: str, filename: str) -> Grid:
		...
		try:
			...
			k = len(col_names)
			grid = Grid(flag, self.path, filename, self.corpus, k, self.synonym_book, clusters)
